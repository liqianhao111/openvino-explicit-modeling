...
if [ "$BUILD_OPENVINO" -eq 1 ]; then
    echo "[BUILD] openvino"
    if ! cmake -S "$ROOT/openvino" -B "$ROOT/openvino/build" -G Ninja -DCMAKE_BUILD_TYPE=Release -DENABLE_PYTHON=ON; then
...
if [ "$BUILD_GENAI" -eq 1 ]; then
    ...
    if ! cmake -S "$ROOT/openvino.genai" -B "$ROOT/openvino.genai/build" -G Ninja -DCMAKE_BUILD_TYPE=Release -DENABLE_PYTHON=ON -DOpenVINO_DIR="$ROOT/openvino/build"; then
