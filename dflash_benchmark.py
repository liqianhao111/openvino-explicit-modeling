...
_SHARED_LIB_EXT = ".dll" if _IS_WINDOWS else ".so"
_DLL_DIR_HANDLES = []
_BOOTSTRAP_LIB_DIRS: List[Path] = []
...
def _find_build_genai_dir(start_dir: Path) -> Optional[Path]:
    ...
    for c in candidates:
        ...
        if any(c.glob("py_openvino_genai*.pyd")) or any(c.glob("py_openvino_genai*.so")):
            return c
    return None


def _find_runtime_lib_dirs(genai_dir: Path) -> List[Path]:
    ...
    for _ in range(8):
        ...
        search = search.parent
        if _IS_WINDOWS:
            # Windows: openvino/bin/intel64/{Release,...}/openvino.dll
            ...
            # Linux: openvino/build/lib/ or openvino/bin/intel64/Release/lib/
            for lib_candidate in [
                search / "openvino" / "build" / "lib",
                search / "openvino" / "bin" / "intel64" / "Release" / "lib",
                search / "openvino" / "bin" / "intel64" / "Release",
            ]:
                if lib_candidate.is_dir() and any(lib_candidate.glob("libopenvino.so*")):
                    entries.append(lib_candidate)
                    break
        ...
        if tbb_root.is_dir():
            if _IS_WINDOWS:
                ...
                for tbb_lib in sorted(tbb_root.glob("*/tbb/lib")):
                    if any(tbb_lib.glob("libtbb.so*")):
                        entries.append(tbb_lib)
                        break
    unique: List[Path] = []
    seen = set()
    for p in entries:
...
def _bootstrap_openvino_genai() -> None:
    global _BOOTSTRAP_LIB_DIRS, _BOOTSTRAP_GENAI_DIR, _BOOTSTRAP_OPENVINO_PY_DIR
    ...
    if local_ov_py_dir is not None:
        ...
        if s not in sys.path:
            sys.path.insert(0, s)
    lib_dirs = _find_runtime_lib_dirs(genai_dir)
    lib_name = "openvino.dll" if _IS_WINDOWS else "libopenvino.so"
    lib_glob = "*.dll" if _IS_WINDOWS else "*.so*"
    has_local = any((d / lib_name).is_file() or any(d.glob(lib_name + "*")) for d in lib_dirs)
    if not has_local:
        spec = importlib.util.find_spec("openvino")
        if spec and spec.origin:
            pkg = Path(spec.origin).resolve().parent
            for c in [pkg / "libs", pkg.parent / "openvino" / "libs"]:
                if c.is_dir() and any(c.glob(lib_glob)):
                    lib_dirs.append(c)
    deduped: List[Path] = []
    seen = set()
    for d in lib_dirs:
    ...
    lib_dirs = deduped
    _BOOTSTRAP_LIB_DIRS = lib_dirs
    if not os.environ.get("OPENVINO_LIB_PATHS", "").strip() and lib_dirs:
        os.environ["OPENVINO_LIB_PATHS"] = _PATH_SEP.join(str(d) for d in lib_dirs)
    if not lib_dirs:
        return
    if _IS_WINDOWS and hasattr(os, "add_dll_directory"):
        ...
        prepend = ";".join(str(d) for d in lib_dirs)
        os.environ["PATH"] = f"{prepend};{existing}" if existing else prepend
    else:
        existing = os.environ.get("LD_LIBRARY_PATH", "")
        prepend = ":".join(str(d) for d in lib_dirs)
        os.environ["LD_LIBRARY_PATH"] = f"{prepend}:{existing}" if existing else prepend
